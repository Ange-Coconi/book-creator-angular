...
@Component({
  ...
  template: `
    <button 
      class="w-full h-44 flex flex-col items-center px-3 py-3 border-2 border-white rounded-md shadow-md hover:opacity-80"
      [routerLink]="bookFeaturePath"
      (click)="handleBookDashboardClick(book)"
      >
...
export class BookDashboardComponent {
  ...
  handleBookDashboardClick(bookClicked: Book) {
