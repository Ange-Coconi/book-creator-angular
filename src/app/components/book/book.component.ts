...
@Component({
  ...
  standalone: true,
  template: `
    <button class="w-full flex justify-left items-center shadow-md hover:opacity-80" (click)="bookClicked.emit(book)" >
      ...
      <p class="ml-4 block text-center text-white px-2 py-2 overflow-x-hidden" >{{title}}</p>
    </button>
  `,
  styles: ``
})
